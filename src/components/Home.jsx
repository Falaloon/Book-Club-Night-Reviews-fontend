import React, { useState, useEffect } from "react";
import axios from "axios";
import { Link } from "react-router-dom";
import Footer from "./Footer";

export default function Home() {
  const [data, setData] = useState([]);
  const [form, setForm] = useState({});
  useEffect(() => {
    //code here
    loadData();
  }, []);

  const loadData = async () => {
    await axios
      .get(import.meta.env.VITE_API_URL + "/products")
      .then((res) => setData(res.data))
      .catch((err) => console.log(err));
  };

  const handleRemove = async (id) => {
    console.log(`Get id: ${id}`);
    await axios
      .delete(import.meta.env.VITE_API_URL + "/products" + "/" + id)
      .then((res) => {
        console.log(res);
        loadData();
      })
      .catch((err) => console.log(err));
  };

  return (
    <div className="font-IBMPlex font-IBMPlexThai relative">
      <p className="text-3xl font-extrabold	 text-center m-5 font-IBMPlex text-[#3E54AC]">
        Book Club Night Reviews
      </p>
      <div className="flex justify-end m-6 fixed top-0 left-0 right-0">
        <a href="/addReview">
          <svg
            viewBox="0 0 24 24"
            width="50px"
            height="50px"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            <g
              id="SVGRepo_tracerCarrier"
              stroke-linecap="round"
              stroke-linejoin="round"
            >
              {" "}
            </g>
            <g id="SVGRepo_iconCarrier">
              {" "}
              <rect width="24" height="24"></rect>{" "}
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M13 9C13 8.44772 12.5523 8 12 8C11.4477 8 11 8.44772 11 9V11H9C8.44772 11 8 11.4477 8 12C8 12.5523 8.44772 13 9 13H11V15C11 15.5523 11.4477 16 12 16C12.5523 16 13 15.5523 13 15V13H15C15.5523 13 16 12.5523 16 12C16 11.4477 15.5523 11 15 11H13V9ZM7.25007 2.38782C8.54878 2.0992 10.1243 2 12 2C13.8757 2 15.4512 2.0992 16.7499 2.38782C18.06 2.67897 19.1488 3.176 19.9864 4.01358C20.824 4.85116 21.321 5.94002 21.6122 7.25007C21.9008 8.54878 22 10.1243 22 12C22 13.8757 21.9008 15.4512 21.6122 16.7499C21.321 18.06 20.824 19.1488 19.9864 19.9864C19.1488 20.824 18.06 21.321 16.7499 21.6122C15.4512 21.9008 13.8757 22 12 22C10.1243 22 8.54878 21.9008 7.25007 21.6122C5.94002 21.321 4.85116 20.824 4.01358 19.9864C3.176 19.1488 2.67897 18.06 2.38782 16.7499C2.0992 15.4512 2 13.8757 2 12C2 10.1243 2.0992 8.54878 2.38782 7.25007C2.67897 5.94002 3.176 4.85116 4.01358 4.01358C4.85116 3.176 5.94002 2.67897 7.25007 2.38782Z"
                fill="#655DBB"
              ></path>{" "}
            </g>
          </svg>
        </a>
      </div>
      <div className="grid grid-cols-3 gap-4 m-6">
        {data.map((item, index) => (
          <div
            key={index}
            id="card"
            className="border-solid border-4 border-[#BFACE2] w-full rounded-lg self-center justify-center p-5"
          >
            <p className="font-bold	">
              Title/ชื่อหนังสื่อ:{" "}
              <span className=" font-normal	">{item.name}</span>
            </p>
            <p className="font-bold	">
              Output/สิ่งที่ได้รับ:{" "}
              <span className=" font-normal	">{item.detail}</span>
            </p>
            <p className="font-bold	">
              Price/ราคา: <span className=" font-normal	">{item.price} ฿</span>
            </p>
            <div className="flex justify-around mt-3">
              <button
                type="button"
                onClick={() => handleRemove(item._id)}
                className="text-white  bg-red-700 hover:bg-red-800  focus:ring-red-300  rounded-full text-sm px-5 py-2.5 text-center  dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
              >
                Delete
              </button>

              <Link to={"/edit/" + item._id}>
                <button
                  type="button"
                  className="text-white  bg-yellow-700 hover:bg-yellow-800  focus:ring-yellow-300 rounded-full text-sm px-5 py-2.5 text-center dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800"
                >
                  Edit
                </button>
              </Link>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
